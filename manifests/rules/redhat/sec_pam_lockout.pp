    ...
    $services.each | $service | {
      ...
      if ($facts['operatingsystemmajrelease'] > '7') {
        ...
        exec { "update authselect pam lockout config retry ${service}":
          ...
          onlyif  => "test -z \"$(grep -E '^\s*auth\s+required\s+pam_faillock.so\s+.*deny=\S+\s*.*$' ${pf_file})\" || test -n \"$(grep -E '^\s*auth\s+required\s+pam_faillock.so\s+.*unlock_time=\S+\s*.*$' ${pf_file})\"",
        }
