...
#
# @example
#   class security_baseline::rules::sec_network_ipv6_redirect {
#       enforce => true,
#       message => 'Test',
#       loglevel => 'info'
#   }
#
# @api private
class security_baseline::rules::sec_network_ipv6_redirects (
  ...
  if($enforce) {
    ...
    if(($fact != '0') or ($fact_default != '0')) {
      notify { 'net.ipv6.conf.all.accept_redirectss':
        message  => $message,
        loglevel => $loglevel,
      }
    }
