...
#
# @example
#   class security_baseline::rules::sles::sec_wlan_interfaces {
#       enforce => true,
#       message => 'Test',
#       log_level => 'info'
#   }
...
class security_baseline::rules::sles::sec_wlan_interfaces (
  ...
  if ($enforce) {
    if(has_key($facts['security_baseline'], 'wlan_status')) {
      if ($facts['security_baseline']['wlan_status'] == 'enabled') {
    ...
    if(
      ($facts['security_baseline']['wlan_interfaces_count'] != 0) or
      ($facts['security_baseline']['wlan_status'] != 'disabled')
    ) {
