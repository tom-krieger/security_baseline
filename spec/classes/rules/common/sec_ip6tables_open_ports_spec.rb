...
describe 'security_baseline::rules::common::sec_ip6tables_open_ports' do
    ...
    enforce_options.each do |enforce|
      context "on #{os}" do
        ...
        let(:params) do
          {
            'enforce' => enforce,
            'message' => 'ip6tables port policies',
            'log_level' => 'warning',
            'firewall_rules' => {
              '100-6 ssh inbound' => {
                'chain' => 'INPUT',
        ...
        it {
          ...
          if enforce
            is_expected.to contain_firewall('100-6 ssh inbound')
              .with(
                'chain'  => 'INPUT',
                'proto'  => 'tcp',
                'dport'  => 22,
            ...
            is_expected.not_to contain_echo('ip6tables-open-ports')
          else
            is_expected.not_to contain_firewall('100-6 ssh inbound')
            is_expected.to contain_echo('ip6tables-open-ports')
              .with(
                'message'  => 'ip6tables port policies',
                'loglevel' => 'warning',
                'withpath' => false,
