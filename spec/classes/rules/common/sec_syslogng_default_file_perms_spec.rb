          ...
          if enforce
            is_expected.to contain_file_line('syslog-ng permissions')
              .with(
                ...
                'line'   => 'options { flush_lines (0); time_reopen (10); log_fifo_size (1000); chain_hostnames(off); flush_lines(0); perm(0640); stats_freq(3600); threaded(yes); use_dns (no); use_fqdn (no); create_dirs (yes); keep_hostname (yes);};',
              )
